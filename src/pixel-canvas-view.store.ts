...
function changeZoom(state: IPixelCanvasView, zoomIn: boolean, xCoord: number, yCoord: number): IPixelCanvasView {
  ...
  if (zoom < MIN_ZOOM || zoom > MAX_ZOOM) {
    ...
    newState.xOffset = calculateZoomOffset(xCoord, newState.xOffset, oldZoom, newZoom, CANVAS_WIDTH);
    newState.yOffset = calculateZoomOffset(yCoord, newState.yOffset, oldZoom, newZoom, CANVAS_HEIGHT);
    return newState;
  }
}
...
function calculateZoomOffset(coord: number, offset: number, oldZoom: number, newZoom: number, size: number): number {
